...
def main(output_as):
	train_file = "misc/media_and_train.csv"
...
train = pd.read_csv("misc/media_and_train.csv")
user_id = train["ID"]
output = [user_id, predictions_age,predictions_gender]
output = zip(*output)
with open("media_output.csv", "wb") as f:
    writer = csv.writer(f)
    output.insert(0,header)
    writer.writerows(output)

# Get rid of duplicates
df = pd.read_csv("media_output.csv")
...
df.to_csv("media_output.csv",index=False)